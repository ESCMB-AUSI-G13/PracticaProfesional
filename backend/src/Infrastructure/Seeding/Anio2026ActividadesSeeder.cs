...
/// <summary>
/// Siembra todas las actividades académicas del año 2026.
///
/// Grupos activos en 2026:
///   Profesorado 2026 (nuevo, Año 1)    — creados por NuevosEstudiantes2026Seeder
///   Trayecto     2026 (nuevo, Año 1)   — creados por NuevosEstudiantes2026TrayectoSeeder
///   Profesorado  2025 (Año 2)          — continúan; 8 desertan durante este año
///   Profesorado  2024 (Año 3)          — continúan (excluye los 8 desertores de 2025 Año 2)
///   Profesorado  2023 (Año 4)          — completan el programa desde 2025 Año 3
///   Trayecto     2025 (Año 2, egresa)  — completan el programa; DesertoresY2 abandonan
///
/// Idempotente: se omite si ya existen EspaciosCurriculares para cursos 2026.
/// </summary>
public static class Anio2026ActividadesSeeder
{
    private const int Anio = 2026;

    // Índices (0-based) dentro de cada comisión de 30 que desertan en Prof 2025 Año 2.
    private static readonly int[] IndicesDesertorProf2025Anio2 = [4, 11, 19, 27];

    private static readonly DateTime[] FechasClase =
    [
        new(2026, 3, 3),  new(2026, 3, 17),
        new(2026, 4, 7),  new(2026, 4, 21),
        new(2026, 5, 5),  new(2026, 5, 19),
        new(2026, 6, 2),  new(2026, 6, 16),
        new(2026, 7, 7),  new(2026, 7, 21),
        new(2026, 8, 4),  new(2026, 8, 18),
        new(2026, 9, 1),  new(2026, 9, 15),
        new(2026, 9, 29), new(2026, 10, 13),
        new(2026, 11, 3), new(2026, 11, 17),
    ];

    private static readonly DateTime FechaParcial1 = new(2026, 5, 20);
    private static readonly DateTime FechaParcial2 = new(2026, 8, 19);
    private static readonly DateTime FechaParcial3 = new(2026, 10, 14);
    private static readonly DateTime FechaFinal    = new(2026, 12, 2);
    private static readonly DateTime FechaEncuesta = new(2026, 11, 17);
    ...
    public static async Task SeedAsync(AppDbContext db, ILogger logger, CancellationToken ct = default)
    {
        ...
        if (cursoIds.Count == 0)
        {
            logger.LogWarning("Anio2026ActividadesSeeder: no hay cursos de 2026.");
            return;
        }

        var desertoresProf2025 = await SeedInscripcionesContinuantes(db, cursoIds, logger, ct);
        logger.LogInformation(
            "Anio2026ActividadesSeeder: InscripcionesContinuantes — OK ({D} desertores Prof2025 Año2 identificados).",
            desertoresProf2025.Count);

        if (await db.EspaciosCurriculares.AnyAsync(ec => cursoIds.Contains(ec.CursoId), ct))
        {
            logger.LogInformation("Anio2026ActividadesSeeder: ya existe, omitido.");
            return;
        }
        ...
        logger.LogInformation("Anio2026ActividadesSeeder: EspaciosCurriculares — OK.");

        await SeedAsistencias(db, inscripciones, cursoAnioMap, desertoresProf2025, ct);
        logger.LogInformation("Anio2026ActividadesSeeder: Asistencias — OK.");

        var examenesPorMateria = await SeedExamenes(db, materias2026, ct);
        logger.LogInformation("Anio2026ActividadesSeeder: Exámenes — OK.");

        await SeedInscripcionesExamen(db, inscripciones, examenesPorMateria, cursoAnioMap, desertoresProf2025, ct);
        logger.LogInformation("Anio2026ActividadesSeeder: InscripcionesExamen — OK.");

        await SeedHistorialAcademico(db, inscripciones, cursos, cursoAnioMap, desertoresProf2025, ct);
        logger.LogInformation("Anio2026ActividadesSeeder: HistorialAcademico — OK.");

        await SeedEncuestas(db, materias2026, inscripciones, cursoAnioMap, desertoresProf2025, ct);
        logger.LogInformation("Anio2026ActividadesSeeder: Encuestas — OK.");

        await UpdateEstados(db, inscripciones, cursoAnioMap, desertoresProf2025, ct);
        logger.LogInformation("Anio2026ActividadesSeeder: Estados — OK.");

        await FixLibreConFinalAprobado(db, cursoIds, ct);
        logger.LogInformation("Anio2026ActividadesSeeder: FixLibreAprobado — OK.");

        await MarcarDesertoresProf2025Anio2(db, desertoresProf2025, ct);
        logger.LogInformation("Anio2026ActividadesSeeder: DesertoresProf2025 marcados — OK.");

        await UpdateAnioEstudiantes(db, cursoIds, ct);
        logger.LogInformation("Anio2026ActividadesSeeder: Anio estudiantes — OK.");

        logger.LogInformation(
            "Anio2026ActividadesSeeder: completado — {N} inscripciones procesadas.", inscripciones.Count);
    }
    ...
    private static async Task<HashSet<int>> SeedInscripcionesContinuantes(
        AppDbContext db, List<int> cursoIds2026, ILogger logger, CancellationToken ct)
    {
        var yaExistentes = (await db.InscripcionesMateria
            .Where(im => cursoIds2026.Contains(im.CursoId))
            .Select(im => new { im.EstudianteId, im.MateriaId, im.CursoId })
            .ToListAsync(ct))
            .Select(x => (x.EstudianteId, x.MateriaId, x.CursoId))
            .ToHashSet();

        var cursos2026Map = (await db.Cursos.Where(c => c.Anio == Anio).ToListAsync(ct))
        ...
        var desertoresProf2025 = new HashSet<int>();

        // ── a) Trayecto 2026 → Año 1 ────────────────────────────────────────────
        var tray2026 = await db.Estudiantes
            .Include(e => e.Usuario)
            .Where(e => e.FechaDeIngreso.Year == 2026 && e.CarreraId == 2)
            .ToListAsync(ct);

        if (materiaMap.TryGetValue((2, 1), out var matTray1))
        {
            foreach (var est in tray2026)
            {
                if (!TryParsarComision(est.Usuario.Legajo, out var com)) continue;
                if (!cursos2026Map.TryGetValue((2, 1, com), out var cursoId)) continue;
        ...
        // ── b) Profesorado 2023 → Año 4 ─────────────────────────────────────────
        var cursosProf2025Y3 = await db.Cursos
            .Where(c => c.Anio == 2025 && c.AnioLectivo == 3 && c.CarreraId == 1)
            .ToListAsync(ct);
        var cursosProf2025Y3Map = cursosProf2025Y3.ToDictionary(c => c.Id, c => c.Comision);
        ...
        var prof2023Cont = (await db.InscripcionesMateria
            .Where(im => cursosProf2025Y3Map.Keys.Contains(im.CursoId)
            ...
            .Select(g => (EstId: g.Key, Comision: cursosProf2025Y3Map[g.First().CursoId]))
            .ToList();
        ...
        if (materiaMap.TryGetValue((1, 4), out var matProf4))
        {
            ...
            {
                if (!cursos2026Map.TryGetValue((1, 4, com), out var cursoId)) continue;
        ...
        // ── c) Profesorado 2024 → Año 3 ─────────────────────────────────────────
        var cursosProf2025Y2 = await db.Cursos
            .Where(c => c.Anio == 2025 && c.AnioLectivo == 2 && c.CarreraId == 1)
            .ToListAsync(ct);
        var cursosProf2025Y2Map = cursosProf2025Y2.ToDictionary(c => c.Id, c => c.Comision);

        var estIds2024 = await db.Estudiantes
            .Where(e => e.FechaDeIngreso.Year == 2024 && e.CarreraId == 1)
            .Select(e => e.Id)
            .ToListAsync(ct);

        var prof2024Cont = (await db.InscripcionesMateria
            .Where(im => cursosProf2025Y2Map.Keys.Contains(im.CursoId)
                && im.Estado != EstadoInscripcion.Baja
                && estIds2024.Contains(im.EstudianteId))
            .Select(im => new { im.EstudianteId, im.CursoId })
            .ToListAsync(ct))
            .GroupBy(x => x.EstudianteId)
            .Select(g => (EstId: g.Key, Comision: cursosProf2025Y2Map[g.First().CursoId]))
            .ToList();

        if (materiaMap.TryGetValue((1, 3), out var matProf3))
        {
            foreach (var (estId, com) in prof2024Cont)
            {
                if (!cursos2026Map.TryGetValue((1, 3, com), out var cursoId)) continue;
        ...
        // ── d) Profesorado 2025 → Año 2 (con 8 desertores distribuidos) ─────────
        if (materiaMap.TryGetValue((1, 2), out var matProf2))
        {
            var prof2025Todos = await db.Estudiantes
                .Include(e => e.Usuario)
                .Where(e => e.FechaDeIngreso.Year == 2025
                         && e.CarreraId == 1
            ...
            {
                if (!cursos2026Map.TryGetValue((1, 2, comision), out var cursoId)) continue;

                var estComision = prof2025Todos
                ...
                for (int i = 0; i < estComision.Count; i++)
                {
                    ...
                    if (IndicesDesertorProf2025Anio2.Contains(i))
                        desertoresProf2025.Add(est.Id);
                }
            }
        }

        // ── e) Trayecto 2025 → Año 2 (excluye DesertoresY1) ────────────────────
        var tray2025Cont = await db.Estudiantes
            .Include(e => e.Usuario)
            .Where(e => e.FechaDeIngreso.Year == 2025
                     && e.CarreraId == 2
        ...
        {
            foreach (var est in tray2025Cont)
            {
                if (!TryParsarComision(est.Usuario.Legajo, out var com)) continue;
                if (!cursos2026Map.TryGetValue((2, 2, com), out var cursoId)) continue;
        ...
        if (nuevas.Count > 0)
        {
            ...
            logger.LogInformation("Anio2026ActividadesSeeder: {N} inscripciones de continuantes creadas.", nuevas.Count);
        }

        return desertoresProf2025;
    }
    ...
    private static async Task SeedAsistencias(
        ...
        Dictionary<int, int> cursoAnioMap, HashSet<int> desertoresProf2025,
        CancellationToken ct)
    {
        ...
        foreach (var insc in inscripciones)
        {
            ...
            bool esDesertor  = desertoresProf2025.Contains(insc.EstudianteId);
        ...
        CancellationToken ct)
    {
        foreach (var insc in inscripciones)
        {
            if (desertoresProf2025.Contains(insc.EstudianteId)) continue;
    ...
    private static async Task SeedHistorialAcademico(
        ...
        HashSet<int> desertoresProf2025, CancellationToken ct)
    {
        ...
        CancellationToken ct)
    {
        foreach (var materia in materias)
        {
            var encuesta = Encuesta.Crear(
                $"Evaluación Docente 2026 — {materia.Nombre}",
            ...
            var respondentes = inscripciones
                .Where(im => im.MateriaId == materia.Id
                    && !desertoresProf2025.Contains(im.EstudianteId)
                    && CondicionEfectiva(
    ...
    // ──────────────────────────────────────────────────────────────────────────────
    private static async Task MarcarDesertoresProf2025Anio2(
        AppDbContext db, HashSet<int> desertoresProf2025, CancellationToken ct)
    {
        if (desertoresProf2025.Count == 0) return;

        await db.Estudiantes
            .Where(e => desertoresProf2025.Contains(e.Id))
    ...
    private static async Task UpdateAnioEstudiantes(
        AppDbContext db, List<int> cursoIds2026, CancellationToken ct)
    {
        var cursos2026 = await db.Cursos
            .Where(c => c.Anio == Anio)
            .ToListAsync(ct);

        foreach (var nivelGroup in cursos2026.GroupBy(c => c.AnioLectivo))
        {
    ...
    // ──────────────────────────────────────────────────────────────────────────────
    /// <summary>
    /// Completa las actividades de Año 1 Trayecto 2026 para los estudiantes creados
    /// por NuevosEstudiantes2026TrayectoSeeder (que corre DESPUÉS del seeder principal).
    /// Guard propio: si ya existen Asistencias para estos estudiantes en 2026, se omite.
    /// </summary>
    public static async Task SeedTrayecto2026Año1CompletarAsync(
        AppDbContext db, ILogger logger, CancellationToken ct = default)
    {
        var cursosTray2026Y1Ids = await db.Cursos
            .Where(c => c.Anio == 2026 && c.AnioLectivo == 1 && c.CarreraId == 2)
            .Select(c => c.Id)
            .ToListAsync(ct);

        if (cursosTray2026Y1Ids.Count == 0)
        {
            logger.LogWarning("SeedTrayecto2026Año1: no hay cursos Trayecto 2026 Año 1.");
            return;
        }
        ...
        if (estIds.Count == 0)
        {
            logger.LogWarning("SeedTrayecto2026Año1: no hay estudiantes Trayecto 2026. Ejecutá NuevosEstudiantes2026TrayectoSeeder primero.");
            return;
        }

        bool yaExiste = await db.Asistencias
            .AnyAsync(a => estIds.Contains(a.EstudianteId) && a.Fecha.Year == 2026, ct);

        if (yaExiste)
        {
            logger.LogInformation("SeedTrayecto2026Año1: actividades ya existen, omitido.");
            return;
        }

        var inscripciones = await db.InscripcionesMateria
            .Include(im => im.Estudiante)
            .Where(im => cursosTray2026Y1Ids.Contains(im.CursoId) && estIds.Contains(im.EstudianteId))
            .ToListAsync(ct);

        if (inscripciones.Count == 0)
        {
            logger.LogWarning("SeedTrayecto2026Año1: sin inscripciones para Trayecto 2026 Año 1.");
            return;
        }
        ...
        await db.SaveChangesAsync(ct);
        logger.LogInformation("SeedTrayecto2026Año1: Asistencias — OK.");

        // InscripcionesExamen
        var trayMateriaIds = inscripciones.Select(im => im.MateriaId).Distinct().ToList();
        var examenesTray = await db.Examenes
            .Where(e => e.FechaExamen.Year == 2026 && trayMateriaIds.Contains(e.MateriaId))
            .ToListAsync(ct);
        ...
        await db.SaveChangesAsync(ct);
        logger.LogInformation("SeedTrayecto2026Año1: InscripcionesExamen — OK.");

        // HistorialAcademico
        var cursoPorId = (await db.Cursos.Where(c => cursosTray2026Y1Ids.Contains(c.Id)).ToListAsync(ct))
            .ToDictionary(c => c.Id);
        foreach (var insc in inscripciones)
        {
            if (!cursoPorId.TryGetValue(insc.CursoId, out var curso)) continue;
        ...
        await db.SaveChangesAsync(ct);
        logger.LogInformation("SeedTrayecto2026Año1: HistorialAcademico — OK.");
        ...
        logger.LogInformation(
            "SeedTrayecto2026Año1: completado — {N} inscripciones procesadas.", inscripciones.Count);
    }
}
