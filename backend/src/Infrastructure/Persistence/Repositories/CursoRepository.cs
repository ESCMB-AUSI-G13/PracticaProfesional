...
public class CursoRepository(AppDbContext context) : ICursoRepository
{
    ...
    public async Task<bool> ExistePorAnioYComisionAsync(int anio, int anioLectivo, string comision, int carreraId, CancellationToken cancellationToken = default)
        => await context.Cursos.AnyAsync(c => c.Anio == anio && c.AnioLectivo == anioLectivo && c.Comision == comision.ToUpperInvariant() && c.CarreraId == carreraId, cancellationToken);

    public async Task<bool> ExistePorAnioYComisionExcluyendoAsync(int anio, int anioLectivo, string comision, int carreraId, int excludeId, CancellationToken cancellationToken = default)
        => await context.Cursos.AnyAsync(c => c.Anio == anio && c.AnioLectivo == anioLectivo && c.Comision == comision.ToUpperInvariant() && c.CarreraId == carreraId && c.Id != excludeId, cancellationToken);
